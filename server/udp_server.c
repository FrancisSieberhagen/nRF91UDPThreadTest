...
int main(void)
{
  ...
  for (;;) {
	...
	//sprintf(&buffer_send[0], "%s %i\0\0", buffer, i);
	sprintf(&buffer_send[0], "%s \0", buffer);

        printf("Send packet to %s:%d\nData(%lu): %s\n\n",ip_client, ntohs(sa.sin_port), strlen(buffer_send), buffer_send);

        bytes_sent = sendto(sock, buffer_send, strlen(buffer_send), 0,(struct sockaddr*)&sa, sizeof sa);
        if (bytes_sent < 0) {
